...
def main() -> None:
    ...
    with open(Path("miniquant_agent", "aea-config.yaml"), "r", encoding="utf-8") as file:
        ...
        if os.getenv("COINGECKO_API_KEY"):
            ...
            config[-1]["models"]["params"]["args"][
                "openai_api_key"
            ] = f"${{str:{os.getenv('OPENAI_API_KEY')}}}"  # type: ignore

    with open(Path("miniquant_agent", "aea-config.yaml"), "w", encoding="utf-8") as file:
